...
export class Game extends Phaser.Scene {
    ...
    const casilla2Layer = map.createLayer("casilla_pantano_pierde_turno", casilla2, 0, 0);
    const casilla3Layer = map.createLayer("casilla_pantano_volver", casilla3, 0, 0);
    const casilla4Layer = map.createLayer("casilla_pantano_pregunta_X", casilla4, 0, 0);
    const casilla5Layer = map.createLayer("casilla_pantano_pregunta_Y", casilla5, 0, 0);
    const casilla6Layer = map.createLayer("casilla_pantano_avanzar", casilla6, 0, 0);
    const casilla7Layer = map.createLayer("casilla_pantano_moneda_X", casilla7, 0, 0);
    const casilla8Layer = map.createLayer("casilla_pantano_moneda_Y", casilla8, 0, 0);
    const casilla9Layer = map.createLayer("casilla_pantano_esquina", casilla9, 0, 0);
    const troncosLayer = map.createLayer("troncos_pantano", troncos, 0, 0);
    
    //bosque
    const casilla10Layer = map.createLayer("casilla_bosque_comun", casilla10, 0, 0);
    const casilla11Layer = map.createLayer("casilla_bosque_pierde_turno", casilla11, 0, 0);
    const casilla12Layer = map.createLayer("casilla_bosque_volver", casilla12, 0, 0);
    const casilla13Layer = map.createLayer("casilla_bosque_pregunta_X", casilla13, 0, 0);
    const casilla14Layer = map.createLayer("casilla_bosque_pregunta_Y", casilla14, 0, 0);
    const casilla15Layer = map.createLayer("casilla_bosque_avanzar", casilla15, 0, 0);
    const casilla16Layer = map.createLayer("casilla_bosque_moneda_X", casilla16, 0, 0);
    const casilla17Layer = map.createLayer("casilla_bosque_moneda_Y", casilla17, 0, 0);
    const casilla18Layer = map.createLayer("casilla_bosque_esquina", casilla18, 0, 0);

    //bosque
    const casilla19Layer = map.createLayer("casilla_aldea_comun", casilla19, 0, 0);
    const casilla20Layer = map.createLayer("casilla_aldea_pierde_turno", casilla20, 0, 0);
    const casilla21Layer = map.createLayer("casilla_aldea_volver", casilla21, 0, 0);
    const casilla22Layer = map.createLayer("casilla_aldea_pregunta_X", casilla22, 0, 0);
    const casilla23Layer = map.createLayer("casilla_aldea_pregunta_Y", casilla23, 0, 0);
    const casilla24Layer = map.createLayer("casilla_aldea_avanzar", casilla24, 0, 0);
    const casilla25Layer = map.createLayer("casilla_aldea_moneda_X", casilla25, 0, 0);
    const casilla26Layer = map.createLayer("casilla_aldea_moneda_Y", casilla26, 0, 0);
    const casilla27Layer = map.createLayer("casilla_aldea_esquina", casilla27, 0, 0);

    //castillo
    const casilla28Layer = map.createLayer("casilla_castillo_comun", casilla28, 0, 0);
    const casilla29Layer = map.createLayer("casilla_castillo_pierde_turno", casilla29, 0, 0);
    const casilla30Layer = map.createLayer("casilla_castillo_volver", casilla30, 0, 0);
    const casilla31Layer = map.createLayer("casilla_castillo_pregunta_X", casilla31, 0, 0);
    const casilla32Layer = map.createLayer("casilla_castillo_pregunta_Y", casilla32, 0, 0);
    const casilla33Layer = map.createLayer("casilla_castillo_avanzar", casilla33, 0, 0);
    const casilla34Layer = map.createLayer("casilla_castillo_moneda_X", casilla34, 0, 0);
    const casilla35Layer = map.createLayer("casilla_castillo_moneda_Y", casilla35, 0, 0);
    const casilla36Layer = map.createLayer("casilla_castillo_esquina", casilla36, 0, 0);
    ...
    //ordenamiento de casillas
      ...
      ruleta = this.add
        ...
        .on("pointerdown", () => {
          ...
          contadorRuleta = 1;
          bloqueoruleta = this.add.image(1280, 25, 'bloqueoruleta').setScale(0.03).setScrollFactor(0)
    ...
    moverDerecha(playerActivo) {
      ...
      let count = 0
        for (let i = players[turno].casilla > 0 ? players[turno].casilla : 0; i <= (players[turno].casilla + dado); i++) {
    ...
    retroceder(playerActivo) {
        ...
        let count = 0
        for (let i = players[turno].casilla - 2; i <= (players[turno].casilla - 2); i++) {
    ...
    pregunta(){
      ...
      { fontFamily: 'Times', fontSize: '36px', color: '#2B2B2B' }).setScrollFactor(0);
   ...
   setTimeout(() => {
    ...
    corroborarCasillas(){
      ...
      if (
        ...
        setTimeout(() => {
          textoTurnoJugador = this.add.text(this.cameras.main.centerX - 200, this.cameras.main.centerY - 350, 'Turno ' + players[turno].jugador,
          { fontFamily: 'Times', fontStyle: 'italic', fontSize: '64px', color: '#000000' });
